...
int B_write(struct B *_B, void *_buffer, int offset) {
	...
	if (_B->_i >= 0)
	    offset = write_raw_varint32(_B->_i, _buffer, offset);
	else
	    offset = write_raw_varint64(_B->_i, _buffer, offset);	    
...
int B_write_with_tag(struct B *_B, void *_buffer, int offset, int tag) {
	...
	/* Write content.*/
	offset = B_write_delimited_to(_B, _buffer, offset);
...
int B_write_delimited_to(struct B *_B, void *_buffer, int offset) {
	...
	new_offset = B_write(_B, _buffer, offset);
...
int B_read_delimited_from(void *_buffer, struct B *_B, int offset) {
	...
	offset = read_raw_varint32(&size, _buffer, offset);
	B_read(_buffer, _B, offset, size + offset);
...
int A_is_default_message(struct A *_A) {
    return B_is_default_message(&_A->_b)
    ;
}
...
int A_write_with_tag(struct A *_A, void *_buffer, int offset, int tag) {
	...
	offset = write_raw_varint32((tag<<3)+2, _buffer, offset);
	/* Write content.*/
	offset = A_write_delimited_to(_A, _buffer, offset);
...
int A_write_delimited_to(struct A *_A, void *_buffer, int offset) {
	...
	new_offset = A_write(_A, _buffer, offset);
	size = new_offset - offset;
	shift = (size > 127) ? 2 : 1;
	for (i = new_offset - 1; i >= offset; -- i)
	    *((char *)_buffer + i + shift) = *((char *)_buffer + i);
...
int A_read_delimited_from(void *_buffer, struct A *_A, int offset) {
	...
	offset = read_raw_varint32(&size, _buffer, offset);
	A_read(_buffer, _A, offset, size + offset);
...
int C_is_default_message(struct C *_C) {
    return B_is_default_message(&_C->_b)
    ;
}
...
int C_write_with_tag(struct C *_C, void *_buffer, int offset, int tag) {
	...
	offset = write_raw_varint32((tag<<3)+2, _buffer, offset);
	/* Write content.*/
	offset = C_write_delimited_to(_C, _buffer, offset);
...
int C_write_delimited_to(struct C *_C, void *_buffer, int offset) {
	...
	new_offset = C_write(_C, _buffer, offset);
	size = new_offset - offset;
	shift = (size > 127) ? 2 : 1;
	for (i = new_offset - 1; i >= offset; -- i)
	    *((char *)_buffer + i + shift) = *((char *)_buffer + i);
...
int C_read_delimited_from(void *_buffer, struct C *_C, int offset) {
	...
	offset = read_raw_varint32(&size, _buffer, offset);
	C_read(_buffer, _C, offset, size + offset);
...
int AddressBook_is_default_message(struct AddressBook *_AddressBook) {
    return _AddressBook->_address_len == 0
...
void Person_init_optional_attributes(struct Person *_Person) {
	_Person->_optEnum = _WORK;
	
	_Person->_optFloat = 1.0;
	
	_Person->_optDouble = 1.0;
	
	_Person->_optBool = 1;
	
	_Person->_optInt32 = 100;
	
	_Person->_optInt64 = 1000000000000;
	
	_Person->_optSInt32 = 100;
	
	_Person->_optSInt64 = 1000000000000;
	
	_Person->_optUInt32 = 100;
	
	_Person->_optUInt64 = 1000000000000;
	
	_Person->_optFixed32 = 100;
	
	_Person->_optFixed64 = 1000000000000;
	
	_Person->_optSFixed32 = 100;
	
	_Person->_optSFixed64 = 1000000000000;
	
	_Person->_optString_len = 4;
	_Person->_optString[0] = 'v';
	_Person->_optString[1] = 'l';
	_Person->_optString[2] = 'a';
	_Person->_optString[3] = 'd';
	
	_Person->_optBytes_len = 3;
	_Person->_optBytes[0] = '1';
	_Person->_optBytes[1] = '2';
	_Person->_optBytes[2] = '3';
...
int Person_is_default_message(struct Person *_Person) {
    return AddressBook_is_default_message(&_Person->_ab)
    
     && _Person->_name1_len == 0
...
int Person_write(struct Person *_Person, void *_buffer, int offset) {
	...
	offset = write_raw_varint32(encode_zig_zag64(_Person->_sid64), _buffer, offset);
	...
	for (sintAttr64_cnt = 0; sintAttr64_cnt < _Person->_sintAttr64_repeated_len; ++ sintAttr64_cnt) {
	    ...
	    offset = write_raw_varint32(encode_zig_zag64(_Person->_sintAttr64[sintAttr64_cnt]), _buffer, offset);
	}
